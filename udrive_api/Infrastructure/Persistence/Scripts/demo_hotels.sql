-- NOTE (release audit): the Unsplash photo URLs were emptied out.
--
-- This script is not a migration — it runs only when an admin presses
-- "Add demo data" — but the rows it creates are live hotel listings that the
-- customer app renders. Each one hotlinked a photograph from
-- images.unsplash.com, so opening the hotel list sent every customer's IP
-- address to a third party that the Privacy Policy does not name. The policy
-- used to name Unsplash, for a fallback path in the app that has since been
-- deleted; rather than re-add a disclosure for demo data, the demo data no
-- longer contacts anybody. An empty image_url renders the built-in hotel icon.
...
VALUES
    ('53000000-0000-0000-0000-000000000001', '52000000-0000-0000-0000-000000000001', 'Deluxe River View', 'King bed with river-facing balcony.', 2, 6, 14500, '', '["King bed","Balcony","Heating","Private bathroom"]'::jsonb, true, now(), now()),
    ('53000000-0000-0000-0000-000000000002', '52000000-0000-0000-0000-000000000001', 'Family Suite', 'Two-room family suite for Kashmir trips.', 5, 3, 22500, '', '["Two rooms","Family seating","Heating","River view"]'::jsonb, true, now(), now()),
    ('53000000-0000-0000-0000-000000000003', '52000000-0000-0000-0000-000000000002', 'Executive Double', 'Modern double room in central Muzaffarabad.', 2, 10, 12000, '', '["Double bed","WiFi","Breakfast","Air conditioning"]'::jsonb, true, now(), now()),
    ('53000000-0000-0000-0000-000000000004', '52000000-0000-0000-0000-000000000003', 'Pine View Family Room', 'Family room with peaceful hillside view.', 4, 5, 13500, '', '["Family beds","Heating","Mountain view","Hot water"]'::jsonb, true, now(), now()),
    ('53000000-0000-0000-0000-000000000005', '52000000-0000-0000-0000-000000000004', 'Standard Valley Room', 'Demo room attached to a pending hotel submission.', 2, 4, 8500, '', '["Double bed","Hot water","Valley view"]'::jsonb, true, now(), now())
ON CONFLICT (id) DO UPDATE SET
