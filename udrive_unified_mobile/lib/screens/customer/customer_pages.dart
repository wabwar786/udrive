...
class PackagesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.fromLTRB(18, 4, 18, 30), children: [
    PremiumCard(color: Color(0xFF0D4337), child: Row(children: [Icon(Icons.auto_awesome_rounded, color: AppColors.accent, size: 34), SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Driver-created Kashmir tours', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 17)), SizedBox(height: 4), Text('Compare packages, drivers and customer-offer options.', style: TextStyle(color: AppText.secondary, fontSize: 12))]))])),
    const SizedBox(height: 16),
    ...tourPackages.map((package) => Padding(padding: EdgeInsets.only(bottom: 14), child: PremiumCard(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PackageDetailScreen(package: package))), padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ClipRRect(borderRadius: BorderRadius.vertical(top: Radius.circular(23)), child: Image.asset(package.image, height: 180, width: double.infinity, fit: BoxFit.cover)), Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [StatusPill(label: '${package.days} days'), SizedBox(width: 7), StatusPill(label: '${package.rating} ★', color: AppColors.accent), Spacer(), Text('PKR ${package.price}', style: TextStyle(fontWeight: FontWeight.w900, color: AppColors.primaryDark, fontSize: 17))]), SizedBox(height: 11), Text(package.title, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 19)), SizedBox(height: 5), Text(package.route, style: TextStyle(color: AppColors.muted, fontSize: 12)), SizedBox(height: 12), Row(children: [Icon(Icons.person_rounded, size: 18, color: AppColors.muted), Text(' ${package.driver}', style: TextStyle(fontWeight: FontWeight.w700)), Spacer(), Text('${package.maxGuests} guests · ${package.vehicle}', style: TextStyle(color: AppColors.muted, fontSize: 11))])]))])))),
  ]);
}
...
class WalletScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(18, 4, 18, 30), children: [
      Container(padding: const EdgeInsets.all(22), decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: const LinearGradient(colors: [Color(0xFF063F32), AppColors.primary])), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Row(children: [Icon(Icons.account_balance_wallet_rounded, color: Colors.white), Spacer(), Text('Udrive Wallet', style: TextStyle(color: AppText.secondary, fontWeight: FontWeight.w700))]), const SizedBox(height: 24), const Text('Available balance', style: TextStyle(color: AppText.secondary)), const SizedBox(height: 4), Text('PKR $balance', style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.w900)), const SizedBox(height: 22), Row(children: [Expanded(child: _WalletAction(icon: Icons.add_rounded, label: context.tr('addMoney'))), const SizedBox(width: 10), Expanded(child: _WalletAction(icon: Icons.send_rounded, label: context.tr('sendMoney')))])])),
      const SizedBox(height: 22),
      SectionHeader(title: context.tr('transactions')),
      const SizedBox(height: 10),
      ...walletTransactions.map((tx) => Padding(padding: EdgeInsets.only(bottom: 10), child: PremiumCard(child: Row(children: [Container(width: 43, height: 43, decoration: BoxDecoration(color: (tx.credit ? AppColors.success : AppColors.secondary).withValues(alpha: .11), borderRadius: BorderRadius.circular(13)), child: Icon(tx.credit ? Icons.south_west_rounded : Icons.north_east_rounded, color: tx.credit ? AppColors.success : AppColors.secondary)), SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(tx.title, style: TextStyle(fontWeight: FontWeight.w900)), Text(tx.date, style: TextStyle(color: AppColors.muted, fontSize: 11))])), Text('${tx.credit ? '+' : '-'} PKR ${tx.amount}', style: TextStyle(fontWeight: FontWeight.w900, color: tx.credit ? AppColors.success : AppColors.navy))])))),
    ]);
  }
}
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(18), children: [
      PremiumCard(color: Color(0xFF0D4337), child: Column(children: [CircleAvatar(radius: 38, backgroundColor: Colors.white, child: Icon(Icons.person_rounded, size: 42, color: AppColors.primaryDark)), SizedBox(height: 12), Text(controller.currentUserName, style: TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text(controller.currentUserPhone, style: TextStyle(color: AppText.secondary, fontSize: 11)), SizedBox(height: 15), Row(mainAxisAlignment: MainAxisAlignment.center, children: [StatusPill(label: 'Customer account', color: AppColors.accent), SizedBox(width: 8), StatusPill(label: controller.currentUser?.accountStatus ?? 'Active', color: AppColors.success)])])),
      const SizedBox(height: 18),
