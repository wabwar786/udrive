...
class TripLocationService {
  ...
  Future<void> start(String bookingId,String status) async {_bookingId=bookingId;_status=status;_timer?.cancel();await flushQueue();final seconds=status=='TripStarted'||status=='Emergency'?10:30;_timer=Timer.periodic(Duration(seconds:seconds),(_)=>capture());await capture();}
