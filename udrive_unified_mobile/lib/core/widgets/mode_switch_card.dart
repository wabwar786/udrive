...
class ModeSwitchCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(decoration:BoxDecoration(gradient:LinearGradient(colors:isHotel?[Color(0xFF24362F),Color(0xFF5E8B3D)]:isDriver?[AppColors.primary,AppColors.secondary]:[AppColors.secondary,Color(0xFF4F46E5)]),borderRadius:BorderRadius.circular(20)),child:Material(color:Colors.transparent,child:InkWell(borderRadius:BorderRadius.circular(20),onTap:()=>_switch(context,c),child:Padding(padding:EdgeInsets.all(15),child:Row(children:[Container(width:46,height:46,decoration:BoxDecoration(color:Colors.white.withValues(alpha:.16),borderRadius:BorderRadius.circular(14)),child:Icon(icon,color:Colors.white,size:25)),SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:TextStyle(color:Colors.white,fontSize:14,fontWeight:FontWeight.w900)),SizedBox(height:3),Text(subtitle,style:TextStyle(color:Colors.white.withValues(alpha:.78),fontSize:10,height:1.3))])),Icon(Icons.arrow_forward_rounded,color:Colors.white,size:19)])))));
  }
