...
public sealed class AdminOperationsController(AdminOperationsService service) : ControllerBase
{
    ...
    [Authorize(Roles="SuperAdmin,Admin,Operations,TourismManager")][HttpPost("destinations/{id:guid}/image")][RequestSizeLimit(10*1024*1024)] public async Task<IActionResult> UploadDestinationImage(Guid id,[FromForm]IFormFile file,CancellationToken ct)=>Result(await service.UploadDestinationImageAsync(User.GetRequiredUserId(),id,file,ct));
    [Authorize(Roles="SuperAdmin,Admin,TourismManager")][HttpDelete("destinations/{id:guid}")] public async Task<IActionResult> DeleteDestination(Guid id,CancellationToken ct)=>Result(await service.DeleteDestinationAsync(User.GetRequiredUserId(),id,ct));
